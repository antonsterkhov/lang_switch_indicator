...
2. Таймер (каждые `120` мс) проверяет раскладку активного окна через `GetKeyboardLayout`.
3. Индикатор показывается:
   - при первом запуске;
   - при смене раскладки;
   - при начале новой сессии набора текста (если пауза была >= 5 секунд).
4. Индикатор автоматически скрывается через `1200` мс.
